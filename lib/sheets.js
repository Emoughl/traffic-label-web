...
/** Danh sách ảnh đang bị đánh dấu DELETED của 1 ngày, kèm fileId lấy từ cột file_path. */
export async function getDeletedRecords(dateStr) {
  const rows = await getAllRows(dateStr);
  const out = [];
  for (const r of rows) {
    if (!r[0]) continue;
    const isDeleted = r[1] === "-1" || r[2] === "DELETED" || r[3] === "DELETED";
    if (!isDeleted) continue;
    const m = String(r[4] || "").match(/\/d\/([^/]+)/);
    out.push({
      filename: r[0],
      fileId: m ? m[1] : null,
      deletedBy: r[5] || "",
...
/** Gỡ dấu DELETED (dùng khi khôi phục ảnh). Dòng sẽ trở về trạng thái chưa gán nhãn. */
export async function unmarkImageDeleted(dateStr, filenames) {
