...
export async function getFileBytes(fileId) {
  ...
  if (cached && now - cached.ts < CACHE_TTL_MS.fileBytes) {
    return cached.value;
  }
  ...
  const value = { data: res.data, mimeType: meta.data.mimeType || "image/jpeg" };
  fileBytesCache.set(fileId, { value, ts: now });
  return value;
}
...
export async function getFileThumbnailUrl(fileId) {
  ...
  return meta.data.thumbnailLink || `https://drive.google.com/thumbnail?id=${encodeURIComponent(fileId)}&sz=w400`;
}

export async function getFileThumbnailBytes(fileId) {
  const thumbUrl = await getFileThumbnailUrl(fileId);
  const res = await fetch(thumbUrl, { headers: { Accept: "image/*" } });
  if (!res.ok) {
    throw new Error(`thumbnail fetch failed: ${res.status}`);
  }
  const buffer = Buffer.from(await res.arrayBuffer());
  return { data: buffer, mimeType: res.headers.get("content-type") || "image/jpeg" };
}
