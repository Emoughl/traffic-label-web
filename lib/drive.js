...
/**
 * Di chuyển file/shortcut an toàn:
 * 1. Thử addParents / removeParents sang folder Deleted.
 * 2. Nếu không đủ quyền di chuyển (403/400), fallback sang trashed: true hoặc removeParents.
 */
async function moveOrTrashFile(drive, fileId, currentParentId, targetFolderId) {
  try {
    ...
    const parentToRemove = currentParentId && parents.includes(currentParentId)
      ? currentParentId
      : (parents.length > 0 ? parents[0] : null);

    // 1. Thử di chuyển sang targetFolderId nếu có
    if (targetFolderId && targetFolderId !== parentToRemove) {
      try {
        const updateParams = {
          fileId,
        ...
        console.warn(
          `[Google Drive] Không thể chuyển file ${fileId} sang ${targetFolderId} (${moveErr.message}), thử trash file...`
        );
      }
    }

    // 2. Fallback: Đưa file/shortcut vào thùng rác (trashed: true)
    try {
    ...
    // 3. Fallback cuối cùng: gỡ khỏi parent hiện tại nếu có
    if (parentToRemove) {
      await drive.files.update({
        fileId,
    ...
    console.error(`[Google Drive] Lỗi xử lý xóa/di chuyển cho ${fileId}:`, err.message);
    throw err;
  }
}
 ...
 * Hỗ trợ cả file thường, shortcut trỏ tới ảnh, và thư mục dạng shortcut.
 */
export async function moveFilesToDeleted(fileIds, dateStr) {
  ...
  if (!dateFolderId) {
    throw new Error(`Không tìm thấy folder cho ngày: ${dateStr}`);
  }

  // Resolve date folder nếu là Shortcut
  const resolvedDateFolder = await resolveShortcutId(drive, dateFolderId);
  const actualDateFolderId = resolvedDateFolder.id;

  // Resolve ROOT_FOLDER_ID
  const root = await resolveShortcutId(drive, ROOT_FOLDER_ID);
  const actualRootFolderId = root.id;

  // 2. Xác định parent để tạo folder Deleted/
  // Ưu tiên parent của dateFolderId, nếu không được thì dùng actualRootFolderId
  ...
  // 3. Tạo Deleted/{dateStr}/ nếu có deletedFolderId
  let deletedDateFolderId = null;
  if (deletedFolderId) {
    deletedDateFolderId = await findOrCreateFolder(drive, deletedFolderId, dateStr);
  }

  // 4. Liệt kê các item trong folder ngày (cả actualDateFolderId lẫn dateFolderId)
  const foldersToCheck = [...new Set([dateFolderId, actualDateFolderId])];
  const allItems = [];

  for (const fId of foldersToCheck) {
    let pageToken;
    do {
      try {
        const res = await drive.files.list({
          q: `'${fId}' in parents and trashed = false`,
          fields: "nextPageToken, files(id, name, mimeType, shortcutDetails, parents)",
          pageSize: 1000,
          pageToken,
          supportsAllDrives: true,
          includeItemsFromAllDrives: true,
        ...
        console.warn(`[Google Drive] Không thể list files trong folder ${fId}:`, e.message);
        break;
      }
    } while (pageToken);
  }

  // 5. Di chuyển hoặc trash từng file
  for (const targetFileId of fileIds) {
    // Tìm item trực tiếp (file nằm ngay trong folder)
    let itemToMove = allItems.find((item) => item.id === targetFileId);

    // Nếu không thấy trực tiếp, tìm shortcut có targetId khớp
    if (!itemToMove) {
      itemToMove = allItems.find(
        (item) =>
          item.mimeType === "application/vnd.google-apps.shortcut" &&
          item.shortcutDetails?.targetId === targetFileId
      );
    }

    const fileIdToProcess = itemToMove ? itemToMove.id : targetFileId;
    const currentParent = itemToMove?.parents?.[0] || actualDateFolderId;

    await moveOrTrashFile(
      drive,
      fileIdToProcess,
      currentParent,
      deletedDateFolderId
