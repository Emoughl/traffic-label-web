...
/**
 * Di chuyển ảnh vào thư mục Deleted/{dateStr} (cùng cấp với thư mục ngày).
 * Nếu thư mục Deleted hoặc Deleted/{dateStr} chưa có sẽ tự tạo.
 *
 * Hỗ trợ cả ảnh thường (direct child) lẫn shortcut trỏ tới ảnh.
 */
export async function moveFilesToDeleted(fileIds, dateStr) {
  ...
  // 2. Lấy parent của folder ngày (để tạo Deleted cùng cấp)
  const parentId = await getParentFolderId(drive, dateFolderId);
  if (!parentId) {
    throw new Error(
      `Không xác định được parent của folder ngày: ${dateStr}`
    );
  }

  // 3. Tìm hoặc tạo Deleted/
  const deletedFolderId = await findOrCreateFolder(
    drive,
    parentId,
    "Deleted"
  );

  // 4. Tìm hoặc tạo Deleted/{dateStr}/
  const deletedDateFolderId = await findOrCreateFolder(
    drive,
    deletedFolderId,
    dateStr
  );

  // 5. Liệt kê tất cả item trong folder ngày (file + shortcut)
  //    để xác định đúng item cần di chuyển
  const allItems = [];
  let pageToken;
  do {
    const res = await drive.files.list({
  ...
  // 6. Di chuyển từng file
  for (const targetFileId of fileIds) {
    // Tìm item trực tiếp (file nằm ngay trong folder)
    let itemToMove = allItems.find(
      (item) => item.id === targetFileId
    );

    if (!itemToMove) {
      // Có thể là shortcut → tìm shortcut có targetId khớp
    ...
    if (itemToMove) {
      await drive.files.update({
        ...
        addParents: deletedDateFolderId,
        removeParents: dateFolderId,
        supportsAllDrives: true,
        fields: "id, parents",
      });

      console.log(
        `[Google Drive] Moved "${itemToMove.name || itemToMove.id}" → Deleted/${dateStr}`
      );
    } else {
      console.warn(
        `[Google Drive] File ${targetFileId} không tìm thấy trong folder ${dateFolderId}, bỏ qua`
      );
    }
  }

  // 7. Cập nhật cache ảnh
  for (const [folderId, entry] of imagesCache.entries()) {
    const remaining = entry.files.filter(
      (f) => !fileIds.includes(f.id)
    );
